...
void Main_trash::updateView_2() {
    ...
    if (ui->typeCheckBox->isChecked()) {
        if (ui->metal_2->isChecked()) {
            queryStr += "AND MATERIAL_TYPE LIKE 'Customer Service%' ";
        } else if (ui->glass_2->isChecked()) {
            queryStr += "AND MATERIAL_TYPE LIKE 'Leadership%' ";
        } else if (ui->paper_2->isChecked()) {
            queryStr += "AND MATERIAL_TYPE LIKE 'Health & Safety%' ";
        } else if (ui->plastic_2->isChecked()) {
            queryStr += "AND MATERIAL_TYPE LIKE 'Project Management%' ";
        }
    }
    ...
    if (ui->recyclableCheckBox->isChecked() && ui->reusableCheckBox->isChecked() && ui->biodegradableCheckBox->isChecked()) {
        queryStr += "ORDER BY CASE WHEN properties LIKE '%Mandatory%' AND properties LIKE '%Certified%' AND properties LIKE '%Optional%' THEN 0 ELSE 1 END, properties";
    }
    else if (ui->recyclableCheckBox->isChecked() && ui->reusableCheckBox->isChecked()) {
        queryStr += "ORDER BY CASE WHEN properties LIKE '%Mandatory%' AND properties LIKE '%Optional%' THEN 0 ELSE 1 END, properties";
    }
    else if (ui->recyclableCheckBox->isChecked() && ui->biodegradableCheckBox->isChecked()) {
        queryStr += "ORDER BY CASE WHEN properties LIKE '%Mandatory%' AND properties LIKE '%Certified%' THEN 0 ELSE 1 END, properties";
    }
    else if (ui->reusableCheckBox->isChecked() && ui->biodegradableCheckBox->isChecked()) {
        queryStr += "ORDER BY CASE WHEN properties LIKE '%Optional%' AND properties LIKE '%Certified%' THEN 0 ELSE 1 END, properties";
    }
    else if (ui->recyclableCheckBox->isChecked()) {
        queryStr += "ORDER BY CASE WHEN properties LIKE '%Mandatory%' THEN 0 ELSE 1 END, properties";
    }
    else if (ui->biodegradableCheckBox->isChecked()) {
        queryStr += "ORDER BY CASE WHEN properties LIKE '%Certified%' THEN 0 ELSE 1 END, properties";
    }
    else if (ui->reusableCheckBox->isChecked()) {
        queryStr += "ORDER BY CASE WHEN properties LIKE '%Optional%' THEN 0 ELSE 1 END, properties";
    }
...
QPieSeries* Main_trash::createRecyclableSeries(const QMap<QString, double>& amounts, double totalAmount) {
    ...
    //Recyclable = Mandatory
    series->setName("Certified");
    series->append("Certified Only", amounts["Certified "] / totalAmount * 100.0);
    series->append("Certified & Mandatory", (amounts["Certified Mandatory "] + amounts["Mandatory Certified "]) / totalAmount * 100.0);
...
QPieSeries* Main_trash::createBiodegradableSeries(const QMap<QString, double>& amounts, double totalAmount) {
    QPieSeries *series = new QPieSeries();
    series->setName("Optional");
    series->append("Optional Only", amounts["Optional "] / totalAmount * 100.0);
    series->append(" Optional & Certified", (amounts["Optional Certified "] + amounts["Certified optional "]) / totalAmount * 100.0);
    return series;
}

QPieSeries* Main_trash::createReusableSeries(const QMap<QString, double>& amounts, double totalAmount) {
    QPieSeries *series = new QPieSeries();
    series->setName("Mandatory");
    series->append("Mandatory Only", amounts["Mandatory "] / totalAmount * 100.0);
    series->append("Mandatory & Optional", (amounts["Mandatory Optional "] + amounts["Optional Mandatory "])  / totalAmount * 100.0);
...
QPieSeries* Main_trash::createALLThree(const QMap<QString, double>& amounts, double totalAmount) {
    QPieSeries *series = new QPieSeries();
    series->setName("ALL three");
        series->append("Optional & Certified & Mandatory", (amounts["Mandatory Optional Certified "]) / totalAmount * 100.0);
return series;
...
void Main_trash::prepareChart(const QString& materialType, const QString& title, const QColor& color1, const QColor& color2, const QColor& color3) {
    ...
    if (query.exec()) {
        ...
        QPieSeries *series4 = createALLThree(amounts, totalAmount);
